...
## [Unreleased]
  ...
  - **群發訊息與訊息中心效能優化 (Broadcast & Message Center UX)** [2026-03-24]:
    - **修復載入與切換的黑畫面崩潰**：在往上滑動或切換聊天對象時，由於後端傳回的部分歷史老舊訊息可能是原生 JSON Object，即使被 `try-catch` 包裹，若函式「正常回傳」了該物件給 React，會導致 React Virtual DOM 延遲渲染時拋出 `Objects are not valid as a React child` 無法捕捉的錯誤並引發全域黑畫面。現已對最底層的文字過濾渲染器強制實施 `String(text)` 型別隔離，徹底拔除 React 崩潰地雷。
