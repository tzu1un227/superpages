...
## [Unreleased]
  ...
  - **群發訊息與訊息中心效能優化 (Broadcast & Message Center UX)** [2026-03-24]:
    - **修復所有載入與切換的黑畫面崩潰地雷**：
      1. 解決了往上滑動時，因新加入的載入動畫元件 (`<CircularProgress>`) 忘記宣告匯入而導致 React 全域拋出 Error #130 並死機黑畫面的烏龍問題。
      2. 將所有由 API 取得的歷史圖文（如 Flex 訊息等原生物件）及潛在遺失的時間戳記強制套上安全保護傘 (`try-catch` 及文字化 `String()` 強制轉型)，防止未序列化的 JSON 物件直接傳給 React 內部 Virtual DOM 渲染引擎而造成的第二層隱性崩潰。
      3. 加入了全域 `ErrorBoundary` (錯誤保護罩)，未來若有其他未知例外將直接在畫面上顯示紅字追蹤碼，不會再讓使用者只看到死當的黑畫面。
