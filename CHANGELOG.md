...
## [Unreleased]
  ...
  - **法則表設計 (Rule Designer)** [2026-03-17]:
    - **統一管理介面**：實作單一頁面管理 `Q_bank` 與 `QA_bank` 規則，支援搜尋、編輯與刪除。
    - **訊息分解與編輯**：自動將規則中的 `msg_rpy` 陣列拆解為獨立訊息塊，支援文字、圖片與 Flex 訊息的個別編輯。
    - **即時預覽 (Live Preview)**：整合 `JourneyPreview` 元件，編輯規則時可即時查看 LINE 聊天室模擬畫面。
    - **視覺化編輯器整合**：對話框內嵌 Flex 訊息視覺化編輯器，點擊即可開啟編輯視窗。
    - **流程圖概覽**：新增 SVG 基礎的規則流程圖，呈現規則間的流入 (state_in) 與流出 (state_out) 關係。
