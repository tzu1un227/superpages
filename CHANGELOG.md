...
## [Unreleased]
  ...
  - **群發訊息與訊息中心效能優化 (Broadcast & Message Center UX)** [2026-03-24]:
    - **修復載入崩潰**：修正訊息中心往上滾動載入舊訊息時，因捲軸位置定位參照 (layoutRef) 未確實初始化綁定，導致 React 渲染出錯產生黑畫面的問題。現在往上捲動不僅不會黑畫面，還能平滑維持使用者原先的閱讀視角。
    - **修復無限重複訊息**：修正了因為後端 API 可能無視 `after` 時間過濾參數而在每秒輪詢時全額回傳，導致前端無腦累加而引發最後幾則訊息「無限重複」的幽靈 Bug。已於前端載入時加上嚴謹的資料列 `Set` 過濾去重機制，只有真的新訊息才會被寫入畫面。
