...
- **客戶中心實作 (Customer Center Implementation)** [2026-04-28]:
  ...
  - **客戶中心：搜尋功能精進** [2026-04-28]:
    - **擴增搜尋範圍**：在客戶資訊的搜尋欄中，除了客戶名稱與信箱外，現在也能夠透過「電話號碼」和「標籤名稱」來搜尋客戶。
    - **移除 ID 搜尋與提示**：依照需求將搜尋提示字串中的 ID 移除，使介面更加直覺並聚焦於實際聯絡資訊。

  - **客戶中心：匯出資料優化** [2026-04-28]:
    - **動態匯出與介面微調**：限制「匯出資料」按鈕僅在「客戶資訊」頁籤中顯示。同時實作 CSV 匯出功能，匯出內容會直接與畫面上當前過濾、搜尋結果同步（What You See Is What You Get）。

  - **客戶中心：進階篩選條件面板實作** [2026-04-28]:
    - **篩選 Modal**：實作了如同設計圖的彈出式篩選面板。
    - **多維度篩選**：支援以「標籤」（可多選）、「加入時間」（從 history 的 follow 事件取得）、「最近互動時間」，以及「是否有手機 / Email」作為組合過濾條件。過濾後的資料將直接與列表及匯出功能同步連動。
