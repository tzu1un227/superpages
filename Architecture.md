...
- **發布策略 (Publish Strategy)**：統一圖文選單建立時的選項。將開放狀態、適用標籤合併為獨立的「發布策略」(publishStrategy)。支援隱藏(hidden)、全體預設(default)、限定(restricted)三種模式。直接寫入各選單草稿設定中，取代原先的群組發佈共用 Modal 邏輯。
